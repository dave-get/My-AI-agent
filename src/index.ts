...
import type { AgentResponse } from "./lib/types/response.js";
import { ErrorCodes } from "./lib/types/response.js";
import {
  categorizeResponse,
  determineConfidence,
} from "./promises/clear-response.js";
import { createCardHtml, createStyledHtml } from "./lib/html-generator.js";
...
// Helper function to determine if response should include HTML
function shouldIncludeHtml(query: string, message: string): boolean {
  const htmlKeywords = ['style', 'format', 'color', 'highlight', 'bold', 'italic', 'html', 'markup'];
  const queryLower = query.toLowerCase();
  const messageLower = message.toLowerCase();
  
  return htmlKeywords.some(keyword => 
    queryLower.includes(keyword) || messageLower.includes(keyword)
  );
}

// Helper function to generate HTML content based on response type
function generateHtmlContent(message: string, toolUsed?: string, structuredData?: any): string {
  if (toolUsed === 'about_me' && structuredData) {
    // Create a card-style layout for structured data
    return createCardHtml(
      `About Me - ${structuredData.category}`,
      message,
      structuredData.category
    );
  }
  
  // Default styled message
  return createStyledHtml(message, {
...
app.post("/agent", async (req: Request, res: Response) => {
  try {
    ...
    const toolUsed = result.tool?.name;

    // Determine response type
    const structuredData = categorizeResponse(message, toolUsed);
    const includeHtml = shouldIncludeHtml(query, message);
    const responseType: AgentResponse["data"]["metadata"]["responseType"] =
      message.length > 0
        ? (includeHtml ? "html" : (toolUsed ? "structured" : "text"))
        : "error";

    // Generate HTML content if needed
    const htmlContent = generateHtmlContent(message, toolUsed, structuredData)
    ...
    const response: AgentResponse = {
      ...
      data: {
        ...
        metadata: {
          ...
          query,
          responseType,
          confidence: determineConfidence(message, toolUsed),
        },
        structuredData:
          responseType === "structured" || responseType === "html"
            ? structuredData
            : undefined,
      },
    };
